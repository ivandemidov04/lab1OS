...
void print_usage(const char* program_name) {
    ...
    std::cerr << "    Valid commands: generate, search, search-chunks, full-benchmark" << std::endl;
}
...
int main(int argc, char* argv[]) {
    ...
    if (emasearch_command == "generate") {
    ...
    else if (emasearch_command == "search") {
        if (argc != 5) {
            std::cout << "Usage: --emasearch-command search <input_file> <target_string>" << std::endl;
            return 1;
        }
        std::string input_file = argv[4];
        std::string target_string = argv[5];
        ExternalMemorySearcher::externalMemorySearch(input_file, target_string, 32);
    }
    ...
    else if (emasearch_command == "full-benchmark") {
        if (argc != 8) {
        ...
        std::string target_string = argv[6];
